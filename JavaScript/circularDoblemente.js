...
class ListaDobleCircular {
  ...
  async randominsert() {
    ...
    try {
      ...
      if (isNaN(item)) throw new Error();
      // Inserta DESPUÉS de la ubicación (índice 0)
    ...
    for (let i = 0; i < loc; i++) {
      temp = temp.siguiente;
      if (temp === this.head && i < loc - 1) { // Verificación de vuelta
  ...
  last_delete() {
    if (this.head === null) {
      console.log("\nLa lista está vacía");
    } else if (this.head === this.tail) { // Solo un nodo (Lógica corregida)
      this.head = null;
      this.tail = null;
      console.log("\nSolo se eliminó un nodo de la lista ...\n");
    } else {
      let puntero = this.tail;
      this.tail = puntero.anterior;
      this.tail.siguiente = this.head;
      this.head.anterior = this.tail;
  ...
  async random_delete() {
    let loc;
    try {
      // El C++ es ambiguo. Asumiré "Eliminar EN la ubicación loc"
      // ya que loc=0 llama a begin_delete().
    ...
    if (puntero === this.tail) { // Lógica corregida
      this.tail = puntero.anterior;
    }
    console.log(`\nNodo eliminado ${loc}`);
  }
